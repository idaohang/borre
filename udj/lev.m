...
%$Revision: 1.1 $  $date: 1997/09/26, 1999/10/06 $
...
for i = 1:m
   ...
   if t1 > 0 & t2 == 0
      obs(i,3) = obs(i,3)+fix(t1,2); 
   end
   if t3 > 0 & t4 == 0
      obs(i,3) = obs(i,3)-fix(t3,2);
   end
end
obs(:,[1 2 4]) = [];
x = lscov(A,obs,cov);
...
disp(' ')
disp('     Point    Elevation  Standard deviation')
disp('  ------------------------------------------')
fprintf('\n')
for i = 1:n
   fprintf('%8.0f %12.4f m %12.4f m\n',free(i),x(i),sigma(i))
end
fclose all;
%%%%%%%%%%%%%%% end lev.m %%%%%%%%%%%%%%%%%%%%%%%%
