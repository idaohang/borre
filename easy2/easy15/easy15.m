...
%$Revision: 1.1 $ Date 2014/11/18
% correction of coefficient matrix A
...
P = 10^8*eye(3); %4         % covariance for state vector
Q = 0.05^2*eye(3); %4	     % covariances of system
...
semilogy(1:q,res(1:q),'-.',...
    1:q,resr(1:q),':',...
    1:q,innov_norm(1:q),'-','linewidth',.5)
...
figure(2);
yl = 1:p-1;
plot(yl,(master_pos(1,:)-master_pos(1,1)*ones(1,p-1))','.', ...
     yl,(master_pos(2,:)-master_pos(2,1)*ones(1,p-1))','x', ...
     yl,(master_pos(3,:)-master_pos(3,1)*ones(1,p-1))','+','Markersize',2)
