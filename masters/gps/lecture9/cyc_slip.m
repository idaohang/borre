% CYC_SLIP Test for cycle slip and repair of receiver clock offset. 
...
for i = 1:20
    [tr_RAW, dt, sv, eof2] = fepoch_0(fid); % _0
    NoSv = size(sv,1);
    obs = grabdat1(fid, NoSv, NoObs_types);
    Obs = obs(:,cols);
    for j = 1:NoSv
        if i == 1
            Obs0 = Obs;
...
%i3 = reshape(i2,size(DP,1),length(i1)/size(DP,1));
...
set(get(gca,'ylabel'),'String',({'Pseudoranges Differenced','over Time [m]'}),...
    'fontsize',8,'color','r','verticalalignment','bottom','rotation',90)   
