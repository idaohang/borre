...
std_dev = 0.010;
c1 = 1; %fobs_typ(Obs_types1,'L1'); % or L1 or L2
c2 = 1; %fobs_typ(Obs_types2,'L1'); % or L1 or L2
c3 = 3; %fobs_typ(Obs_types1,'P1');
c4 = 3; %fobs_typ(Obs_types2,'P1');
num_of_matches = 0;
...
if num_of_matches < 2,return, end; % break
...
for j = 1:num_of_matches
    ...
    [rhoj1, X_Satj1] = ...
        get_rho(time1, obs1(loc11,c3), Eph(:,k), X_ista1);
    % We store satellite positions for forming partials
    X_Sat1(:,j) = X_Satj1;
    rho1(j) = rhoj1;
    loc12 = location_2(j);  %  Sat j, sta 2
    [rhoj2, X_Satj2] = ...
        get_rho(time2, obs2(loc12,c4), Eph(:,k), X_ista2);
    X_Sat2(:,j) = X_Satj2;
    rho2(j) = rhoj2;
